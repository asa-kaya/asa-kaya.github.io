...
export default class ModalDiv extends React.Component<ModalDivProps, { hidden: boolean }> {
    ...
    setHidden = (value: boolean) => {
        ...
        if (!this.state.hidden && this.props.onClose)
            this.props.onClose();
    }

    render = () => (
        <div className={`relative z-50 transition-all ease-out duration-300 ${this.state.hidden ? "opacity-0 invisible" : "opacity-100 visible"}`} role="dialog" aria-modal="true">
